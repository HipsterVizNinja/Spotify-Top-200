...
for filename in glob('wildcard filename match.extension'):
    ...
    df.to_csv('past save to path'+filename,index=False,columns=['chart_position','chart_date','song','performer','streams','spotify_uri','song_id','region','time_on_chart','consecutive_days','previous_day','peak_position','worst_position'],encoding='utf-8')
    print(filename)
